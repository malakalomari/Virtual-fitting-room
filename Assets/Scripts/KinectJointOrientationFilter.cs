...
public class KinectJointOrientationFilter : MonoBehaviour
{
    ...
    {
        public Windows.Kinect.Vector4 m_vRawRotation;
        public Windows.Kinect.Vector4 m_vFilteredRotation;
        //	public CameraSpacePoint m_vRawPosition;
        //	public CameraSpacePoint m_vFilteredPosition;

        public Windows.Kinect.Vector4 m_vTrend;
        public int m_dwFrameCount;
    }

    // Holt Double Exponential Smoothing filter
    Windows.Kinect.Vector4[] m_pFilteredJoints;
    FilterDoubleExponentialData[] m_pHistory;
    //	FilterDoubleExponentialData[] m_rHistory;
    float m_fSmoothing;
    float m_fCorrection;
    float m_fPrediction;
    ...
    public void Reset(float fSmoothing = 0.25f, float fCorrection = 0.25f, float fPrediction = 0.25f, float fJitterRadius = 0.03f, float fMaxDeviationRadius = 0.05f)
    {
        ...
        for (int i = 0; i < Body.JointCount; i++)
        {
            ...
            m_pHistory[i].m_vFilteredRotation.X = 0.0f;
            m_pHistory[i].m_vFilteredRotation.Y = 0.0f;
            m_pHistory[i].m_vFilteredRotation.Z = 0.0f;
            m_pHistory[i].m_vFilteredRotation.W = 0.0f;

            m_pHistory[i].m_vRawRotation.X = 0.0f;
            m_pHistory[i].m_vRawRotation.Y = 0.0f;
            m_pHistory[i].m_vRawRotation.Z = 0.0f;
            m_pHistory[i].m_vRawRotation.W = 0.0f;
    ...
    //--------------------------------------------------------------------------------------
    bool JointPositionIsValid(Windows.Kinect.Vector4 vJointPosition)
    {
    ...
    public Windows.Kinect.Vector4[] GetFilteredRotationJoints()
    {
        return m_pFilteredJoints;
    }

    public Windows.Kinect.Vector4 GetFilteredRotationJoint(int jt)
    {
    ...
    Windows.Kinect.Vector4 CSVectorZero()
    {
        Windows.Kinect.Vector4 point = new Windows.Kinect.Vector4();
    ...
    Windows.Kinect.Vector4 CSVectorAdd(Windows.Kinect.Vector4 p1, Windows.Kinect.Vector4 p2)
    {
        Windows.Kinect.Vector4 sum = new Windows.Kinect.Vector4();
    ...
    Windows.Kinect.Vector4 CSVectorScale(Windows.Kinect.Vector4 p, float scale)
    {
        Windows.Kinect.Vector4 point = new Windows.Kinect.Vector4();
    ...
    Windows.Kinect.Vector4 CSVectorSubtract(Windows.Kinect.Vector4 p1, Windows.Kinect.Vector4 p2)
    {
        Windows.Kinect.Vector4 diff = new Windows.Kinect.Vector4();
    ...
    float CSVectorLength(Windows.Kinect.Vector4 p)
    {
        return Convert.ToSingle(Math.Sqrt(p.X * p.X + p.Y * p.Y + p.Z * p.Z + p.W * p.W));
    }

    void UpdateJoint(Body body, JointType jt, TRANSFORM_SMOOTH_PARAMETERS smoothingParams)
    {
        Windows.Kinect.Vector4 vPrevRawRotation;
        Windows.Kinect.Vector4 vPrevFilteredRotation;
        Windows.Kinect.Vector4 vPrevTrend;
        Windows.Kinect.Vector4 vRawRotation;
        Windows.Kinect.Vector4 vFilteredRotation;
        Windows.Kinect.Vector4 vPredictedRotation;
        Windows.Kinect.Vector4 vDiff;
        Windows.Kinect.Vector4 vTrend;
        ...
        vRawRotation = body.JointOrientations[jt].Orientation;

        vPrevFilteredRotation = m_pHistory[(int)jt].m_vFilteredRotation;

        vPrevTrend = m_pHistory[(int)jt].m_vTrend;

        vPrevRawRotation = m_pHistory[(int)jt].m_vRawRotation;


        bJointIsValid = JointPositionIsValid(vRawRotation);
        ...
        {
            vFilteredRotation = vRawRotation;
            vTrend = CSVectorZero();
            m_pHistory[(int)jt].m_dwFrameCount++;
        }
        else if (m_pHistory[(int)jt].m_dwFrameCount == 1)
        {
            vFilteredRotation = CSVectorScale(CSVectorAdd(vRawRotation, vPrevRawRotation), 0.5f);

            vDiff = CSVectorSubtract(vFilteredRotation, vPrevFilteredRotation);
        ...
        else
        {
            ...
            vDiff = CSVectorSubtract(vRawRotation, vPrevFilteredRotation);
            fDiff = CSVectorLength(vDiff);

            if (fDiff <= smoothingParams.fJitterRadius)
            {
                vFilteredRotation = CSVectorAdd(CSVectorScale(vRawRotation, fDiff / smoothingParams.fJitterRadius),
                                                CSVectorScale(vPrevFilteredRotation, 1.0f - fDiff / smoothingParams.fJitterRadius));
            }
            else
            {
                vFilteredRotation = vRawRotation;
            ...
            vFilteredRotation = CSVectorAdd(CSVectorScale(vFilteredRotation, 1.0f - smoothingParams.fSmoothing),
                                            CSVectorScale(CSVectorAdd(vPrevFilteredRotation, vPrevTrend), smoothingParams.fSmoothing));
        ...
        vPredictedRotation = CSVectorAdd(vFilteredRotation, CSVectorScale(vTrend, smoothingParams.fPrediction));

        // Check that we are not too far away from raw data
        vDiff = CSVectorSubtract(vPredictedRotation, vRawRotation);
        fDiff = CSVectorLength(vDiff);

        if (fDiff > smoothingParams.fMaxDeviationRadius)
        {
            vPredictedRotation = CSVectorAdd(CSVectorScale(vPredictedRotation, smoothingParams.fMaxDeviationRadius / fDiff),
                                             CSVectorScale(vRawRotation, 1.0f - smoothingParams.fMaxDeviationRadius / fDiff));
        }

        // Save the data from this frame
        m_pHistory[(int)jt].m_vRawRotation = vRawRotation;

        m_pHistory[(int)jt].m_vFilteredRotation = vFilteredRotation;

        m_pHistory[(int)jt].m_vTrend = vTrend;

        // Output the data
        m_pFilteredJoints[(int)jt] = vPredictedRotation;
    }
}